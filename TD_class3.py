...
fn_train = 'train_new20180502_allFeatures.csv'
fn_test = 'test_new20180502_allFeatures.csv'
...
p = 0.005
train_df = pd.read_csv(path+fn_train, 
                        header=0,
                        #~ usecols=cols,
...
lgb_params = {
        ...
        'metric':metrics,
        'learning_rate': 0.1,
        'num_leaves': 20,  # we should let it be smaller than 2^(max_depth)
        ...
        'nthread': 15,
        'verbose': 0,
        'scale_pos_weight':1.0, #99.7, # because training data is extremely unbalanced 
        'metric':metrics
}
...
test_df = pd.read_csv(path+fn_test, 
                        header=0,
                        #~ usecols=cols,
                        skiprows=lambda i: i>0 and rnd.random() > p
                        )
...
print("writing...")
sub.to_csv("prediction41.csv", index=False, float_format='%.9f')
